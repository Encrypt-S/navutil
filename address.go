...
func NewAddressPubKey(serializedPubKey []byte, net *chaincfg.Params) (*AddressPubKey, error) {
	pubKey, err := navec.ParsePubKey(serializedPubKey, navec.S256())
	if err != nil {
		return nil, err
	}

	// Set the format of the pubkey.  This probably should be returned
	// from navec, but do it here to avoid API churn.  We already know the
...
func (a *AddressPubKey) PubKey() *navec.PublicKey {
	return a.pubKey
}
