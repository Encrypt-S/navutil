...
func (cs *CoinSet) TotalValue() (value navutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue navutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue navutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount navutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue navutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue navutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue navutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue navutil.Amount, coins []Coin) (Coins, error) {
...
// navutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() navutil.Amount {
	return navutil.Amount(c.txOut().Value)
}
