...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := navec.S256().ScalarBaseMult(k.key)
		pubKey := navec.PublicKey{Curve: navec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(navec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := navec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := navec.ParsePubKey(k.key, navec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := navec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := navec.PublicKey{Curve: navec.S256(), X: childX, Y: childY}
...
// ECPubKey converts the extended key to a navec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*navec.PublicKey, error) {
	return navec.ParsePubKey(k.pubKeyBytes(), navec.S256())
}

// ECPrivKey converts the extended key to a navec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*navec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := navec.PrivKeyFromBytes(navec.S256(), k.key)
	return privKey, nil
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(navec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(navec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := navec.ParsePubKey(keyData, navec.S256())
		if err != nil {
			return nil, err
		}
	}
