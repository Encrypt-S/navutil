...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := navutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(navutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := navutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := navutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := navutil.NewBlockFromBytes(blockBytes)
	if err != nil {
